...
from util import create_config, log
...
log("Welcome to audio classification")
...
train_set = UrbanSoundDataset(path_to_UrbanSound8K_csv, path_to_UrbanSound8K_audio, range(1,10), hop_length=config.getint('hop_length'), number_of_mel_filters=config.getint('number_of_mel_filters'),
                              resample_freq=config.getint('resample_freq'), return_audio=False, is_train=True)
test_set = UrbanSoundDataset(path_to_UrbanSound8K_csv, path_to_UrbanSound8K_audio, [10], hop_length=config.getint('hop_length'), number_of_mel_filters=config.getint('number_of_mel_filters'),
                             resample_freq=config.getint('resample_freq'), return_audio=True, is_train=False)
log("Train set size: " + str(len(train_set)))
log("Test set size: " + str(len(test_set)))
...
optimizer = optim.Adam(model.parameters(), lr = config.getfloat('base_lr'))
scheduler = optim.lr_scheduler.StepLR(optimizer, step_size = config.getint('number_of_epochs')//3, gamma = 0.7)
criterion = nn.CrossEntropyLoss()

device = torch.cuda.current_device() if torch.cuda.is_available() else torch.device('cpu')
model.to(device)

log('Device to use: {}'.format(device))
